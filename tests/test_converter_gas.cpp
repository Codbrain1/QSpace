...
void INIT_CONVERTER_PARAMS(std::string_view files_type)
{
    ...
    if (files_type == ParametrsList::is_bin_ifiles) {
        for (const auto& entry : std::filesystem::directory_iterator(INITIAL_DATA_PATH / "bin/GAS")) {
        ...
        for (const auto& entry : std::filesystem::directory_iterator(INITIAL_DATA_PATH / "txt/GAS")) {
            // Пропускаем подкаталоги, если нужны только файлы
            if (entry.is_regular_file()) {
                // entry.path() – полный путь (можно .string(), .u8string() и т.д.)
                pathes.emplace_back(entry.path());
...
void INIT_DATASTORAGE(DataStorage& datastorage)
{
    ...
    datastorage.setup_columns({
        std::string(ParametrsList::X),
        std::string(ParametrsList::Y),
        std::string(ParametrsList::Z),
        std::string(ParametrsList::Rho),
        std::string(ParametrsList::VX),
        std::string(ParametrsList::VY),
        std::string(ParametrsList::VZ),
        std::string(ParametrsList::e),
        std::string(ParametrsList::m),
        std::string(ParametrsList::ind_sph),
        std::string(ParametrsList::t_MCYS),
    });
}
...
void START_CONVERT(std::string_view ifile_type, std::string_view ofile_type)
{
    ...
    DataStorage datastorage(GAS_TEST::pathes);
    ...
    Z_out_par.push_back(std::string(ParametrsList::Z_outParamT));
    Z_out_par.push_back(std::string(ParametrsList::Z_outParamLgT));
...
bool compare_surfer6_bin(const std::filesystem::path& gen, const std::filesystem::path& ref)
{
    ...
    const float EPS = GAS_TEST::FLOAT_EPS;
...
ParsedFilename parse_filename(const std::string& filename)
{
    ...
    std::string stem = path.stem().string();      // GAS_XY_LgRho_2130.694666
    ...
    if (parts[0] != "GAS") {
        throw std::runtime_error("Ожидался префикс GAS: " + filename);
    }
...
std::filesystem::path find_reference(const std::filesystem::path& generated, std::string_view output_type)
{
    ...
    // Путь к эталону: REFERENCE_ROOT / bin|txt / GAS / XY / LgRho / <полное_имя_файла>
    return REFERENCE_ROOT / ext_dir / "GAS" / p.projection / p.param / p.full_name;
}
