...
void Converter_Gas::convert()
{
    ...
    for (size_t file_type = 0; file_type < Z_grd_list.size(); ++file_type) {
        if (Z_grd_list[file_type] == ParametrsList::Z_outParams_Rho) {
            ...
            calculate_Rho(projection1, projection2, projection3, [&ind_sph](int idx) { return ind_sph[idx] == 0; });
            create_output_directory("GAS", xy, "Rho", files_size);
        } else if (Z_grd_list[file_type] == ParametrsList::Z_outParams_LgRho) {
            //-----------------------Вычисление значений--------------------------
            Z.push_back(std::vector<double>());
            const auto& ind_sph = data.get_ind_sph();
            calculate_LgRho(projection1, projection2, projection3, [&ind_sph](int idx) { return ind_sph[idx] == 0; });
            ...
            Z.push_back(std::vector<double>());
            const auto& ind_sph = data.get_ind_sph();
            calculate_Vfi(projection1, projection2, projection3, [&ind_sph](int idx) { return ind_sph[idx] == 0; });
            ...
            Z.push_back(std::vector<double>());
            const auto& ind_sph = data.get_ind_sph();
            calculate_Vr(projection1, projection2, projection3, [&ind_sph](int idx) { return ind_sph[idx] == 0; });
            ...
            Z.push_back(std::vector<double>());
            const auto& ind_sph = data.get_ind_sph();
            calculate_T(projection1, projection2, projection3, [&ind_sph](int idx) { return ind_sph[idx] == 0; });
            ...
            Z.push_back(std::vector<double>());
            const auto& ind_sph = data.get_ind_sph();
            calculate_LgT(projection1, projection2, projection3, [&ind_sph](int idx) { return ind_sph[idx] == 0; });
            ...
            create_output_directory("GAS", xy, "V_module", files_size);
            ...
            create_output_directory("GAS", xy, "c_r", files_size);
            ...
            create_output_directory("GAS", xy, "c_phi", files_size);
            ...
            create_output_directory("GAS", xy, "c_z", files_size);
        } else {
