...
#include <filesystem>
#include <vector>
Converter_Stars::Converter_Stars(DataStorage& _data, ParametrsList::iniConstants& c, count_cell _Nbxy,
        ...
        if (Z_grd_list[file_type] == ParametrsList::Z_outParams_Rho) {
            ...
            for (size_t i = 0; i < file_names.size(); ++i) {
                ofiles_names.push_back(output_directory / dir /
                                       ("STARS_" + xy + "_" + dir + "_t_" + std::to_string(data.get_t()[i] * l_t) + ".grd"));
            }
...
void Converter_Stars::calculate_LgSigma(std::vector<double>& projection1, std::vector<double>& projection2)
{
...
void Converter_Stars::calculate_Sigma(std::vector<double>& projection1, std::vector<double>& projection2)
{
    const auto& Ns = data.get_Ns();  // число строк в каждом файле
    const auto& m_p = data.get_m();
    const auto& offests = data.get_offsets();
...
void Converter_Stars::calculate_LgRho(std::vector<double>& projection1, std::vector<double>& projection2,
...
void Converter_Stars::calculate_Rho(std::vector<double>& projection1, std::vector<double>& projection2,
                                    std::vector<double>& projection3)
{
    const auto& Ns = data.get_Ns();  // число строк в каждом файле
    const auto& m_p = data.get_m();
...
void Converter_Stars::calculate_V_projection(std::vector<double>& projection1, std::vector<double>& projection2,
